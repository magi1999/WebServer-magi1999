...
HttpData::HttpData(EventLoop *loop, int connfd):loop_(loop),
...
nowReadPos_(0),state_(STATE_PARSE_URI),hState_(H_START),keepAlive_(false) 
{
    channel_->setReadHandler(bind(&HttpData::handleRead, this));
    channel_->setWriteHandler(bind(&HttpData::handleWrite, this));
    channel_->setConnHandler(bind(&HttpData::handleConn, this));
    channel_->setErrorHandler(bind(&HttpData::handleError2, this));
}
...
void HttpData::handleRead() 
{
    ...
    do 
    {
        ...
        if (state_ == STATE_PARSE_URI) 
        {
            ...
            else if (flag == PARSE_URI_ERROR) 
            {
                ...
                inBuffer_.clear();
                error_ = true;
                handleError(fd_, 400, "Bad Request2");
                break;
            } 
            else
        ...
        if (state_ == STATE_PARSE_HEADERS) 
        {
            ...
            else if (flag == PARSE_HEADER_ERROR) 
            {
                perror("3");
                error_ = true;
                handleError(fd_, 400, "Bad Request3");
                break;
            }
    ...
    if (!error_) 
    {
      ...
      events_ |= EPOLLIN;
        ...
        LOG<<"HTTPDATA READ END:"<<inBuffer_;
}
...
HeaderState HttpData::parseHeaders() {
  ...
  bool notFinish = true;
  size_t i = 0;
  for (i=0; i < str.size() && notFinish; ++i) {
    switch (hState_) {
      case H_START: {
        if (str[i] == '\n' || str[i] == '\r') break;
        hState_ = H_KEY;
  ...
  if (method_ == METHOD_POST) {
    ...
    if (fileName_ == "hello") {
      outBuffer_ =
        ...
        LOG<<" outputbuffer hello";
      return ANALYSIS_SUCCESS;
    }
    if (fileName_ == "favicon.ico") {
      header += "Content-Type: image/png\r\n";
      header += "Content-Length: " + to_string(sizeof favicon) + "\r\n";
      header += "Server: Magi's Web Server\r\n";

      header += "\r\n";
      outBuffer_ += header;
      outBuffer_ += string(favicon, favicon + sizeof favicon);
              LOG<<" outputbuffer favicon.ico";
      return ANALYSIS_SUCCESS;
    }

    struct stat sbuf;
    if (stat(fileName_.c_str(), &sbuf) < 0) //stat函数用来获取指定路径的文件或者文件夹的信息。
    {
      header.clear();
...
void HttpData::handleError(int fd, int err_num, string short_msg) 
{
  ...
  body_buff += "<hr><em> Magi's Web Server</em>\n</body></html>";
  header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
  header_buff += "Content-Type: text/html\r\n";
  header_buff += "Connection: Close\r\n";
  header_buff += "Content-Length: " + to_string(body_buff.size()) + "\r\n";
  header_buff += "Server: Magi's Web Server\r\n";
  header_buff += "\r\n";
  // 错误处理不考虑writen不完的情况
